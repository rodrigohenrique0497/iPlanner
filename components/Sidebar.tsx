...
const Sidebar: React.FC<SidebarProps> = ({ currentView, setView, user, onLogout, isOpen, onClose }) => {
  ...
  return (
    <>
      ...
      `}>
        {/* Logo / Header Premium - Compactado verticalmente */}
        ...
        {/* Menu Principal - Flex-1 com espaçamento dinâmico */}
        <nav className="flex-1 px-4 py-2 space-y-0.5 overflow-hidden flex flex-col justify-center">
          <p className="text-[9px] font-black uppercase tracking-[0.25em] text-theme-muted opacity-50 mb-3 ml-4">Navegação</p>
            ...
            {navItems.map((item) => {
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center space-x-3.5 px-4 py-2.5 rounded-xl transition-all duration-300 group relative ${
                    isActive
                ...
                >
                  <span className={`material-symbols-outlined !text-[20px] ${isActive ? 'scale-105' : 'group-hover:scale-110'} transition-transform`}>
                    {item.icon}
                  </span>
        ...
        {/* Bloco de Conta - Ajustado para base fixa */}
        <div className="p-4 mt-auto shrink-0">
          <div className="bg-theme-card/60 backdrop-blur-md rounded-[2rem] p-1.5 border border-theme-border/50 shadow-sm flex items-center gap-2">
            <button 
              onClick={() => handleNavClick('settings')}
              className={`flex-1 flex items-center gap-2.5 p-2 rounded-[1.75rem] transition-all hover:bg-theme-accent-soft group/profile ${currentView === 'settings' ? 'bg-theme-accent-soft' : ''}`}
            >
              <div className="w-8 h-8 rounded-full border-2 border-theme-accent/10 overflow-hidden shrink-0 shadow-sm group-hover/profile:border-theme-accent transition-colors">
                <img src={user.avatar} alt={user.name} className="w-full h-full object-cover" />
              </div>
              <div className="text-left min-w-0">
                <p className="text-[11px] font-black text-theme-text truncate leading-none">{user.name.split(' ')[0]}</p>
                <p className="text-[8px] font-bold text-theme-muted uppercase tracking-tighter opacity-50">Assinante</p>
              </div>
            </button>

            <button 
              onClick={onLogout}
              className="w-10 h-10 flex items-center justify-center rounded-2xl text-theme-muted hover:text-rose-500 hover:bg-rose-500/10 transition-all shrink-0 active:scale-90"
            >
              <span className="material-symbols-outlined !text-xl">logout</span>
            </button>
          </div>
          <p className="text-[7px] text-center mt-3 font-black text-theme-muted uppercase tracking-[0.4em] opacity-30">Premium Edition</p>
        </div>
      </aside>
    </>
  );
