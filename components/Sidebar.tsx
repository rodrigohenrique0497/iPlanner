...
const Sidebar: React.FC<SidebarProps> = ({ currentView, setView, user, onLogout, isOpen, onClose }) => {
  ...
  // Lógica de Rank baseada em data de entrada (Placeholder Premium)
  ...
  return (
    <>
      ...
      `}>
        ...
        {/* Menu Principal - Scrollable com margens otimizadas */}
        <nav className="flex-1 px-4 py-2 overflow-y-auto no-scrollbar">
          <p className="text-[9px] font-black uppercase tracking-[0.3em] text-theme-muted opacity-40 mb-3 ml-4">Navegação</p>
        ...
        {/* Footer Fixo Otimizado - Conta e Sair */}
        <div className="p-4 shrink-0 border-t border-theme-border/30 bg-theme-card/10 space-y-2">
          {/* Rank e Perfil Compacto */}
          <button 
            onClick={() => handleNavClick('settings')}
            className={`w-full flex items-center gap-3 p-2.5 rounded-2xl transition-all group/profile border border-transparent ${currentView === 'settings' ? 'bg-theme-accent text-theme-card' : 'hover:bg-theme-accent-soft'}`}
          >
            ...
            <div className="text-left min-w-0">
              ...
              <div className="flex items-center gap-1 mt-1">
                <span className={`text-[8px] font-black uppercase tracking-widest px-1.5 py-0.5 rounded bg-theme-accent/10 border border-theme-accent/10 ${currentView === 'settings' ? 'text-theme-card border-white/20' : 'text-theme-accent'}`}>
                  {getRank()}
                </span>
              </div>
            </div>
          ...
          <button 
            onClick={onLogout}
            className="w-full flex items-center gap-3 px-4 h-11 rounded-xl text-rose-500 hover:bg-rose-500/10 transition-all group active:scale-[0.98]"
          >
            <span className="material-symbols-outlined !text-[20px]">logout</span>
            <span className="text-[10px] font-extrabold uppercase tracking-[0.1em]">Sair da conta</span>
          </button>
        </div>
      </aside>
    </>
