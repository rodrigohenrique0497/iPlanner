    ...
    <div className="px-6 py-6 md:px-12 md:py-10 space-y-10 page-transition max-w-7xl mx-auto pb-32">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6">
        <div className="space-y-1">
          ...
          <p className="text-theme-muted font-bold uppercase tracking-[0.3em] text-[10px] opacity-80">{fullDate}</p>
        </div>
            ...
            {hours.map(h => {
              ...
              return (
                <div key={h} className="flex gap-5 group items-center">
                  {/* Horário com contraste garantido */}
                  <span className="w-12 text-[11px] font-bold text-theme-muted opacity-70 tracking-widest text-center">{h}:00</span>
                  ...
                  {isCreating ? (
                    <form onSubmit={(e) => handleQuickAdd(e, h)} className="flex-1 relative animate-in fade-in zoom-in-95 duration-200">
                      <input 
                        ...
                        className="w-full h-16 rounded-2xl border-2 border-theme-accent bg-theme-card px-6 font-bold text-sm outline-none shadow-glow text-theme-text"
                      />
                      <button 
                        type="submit"
                    ...
                    <div 
                      ...
                      className={`flex-1 h-16 rounded-2xl border transition-all cursor-pointer flex items-center px-6 relative group/slot ${
                        ...
                        : 'border-theme-border bg-theme-card hover:border-theme-accent/30 hover:bg-theme-accent-soft/30'
                      }`}
                    >
                      {task ? (
                        <div className="flex justify-between items-center w-full">
                          <span className={`font-bold text-[15px] truncate ${task.completed ? 'line-through' : ''}`}>{task.title}</span>
                        ...
                        <div className="flex justify-between items-center w-full transition-opacity">
                          <span className="text-[10px] font-black uppercase text-theme-accent opacity-40 group-hover/slot:opacity-100 md:opacity-0 transition-opacity tracking-widest">+ Agendar</span>
                          <span className="material-symbols-outlined !text-xl text-theme-accent opacity-20 group-hover/slot:opacity-100 md:opacity-0 transition-opacity">add_circle</span>
                        </div>
                      )}
                    </div>
                  )}
        ...
        <div className="lg:col-span-5">
          <div className="bg-theme-card p-10 rounded-planner border border-theme-border shadow-premium space-y-10 lg:sticky lg:top-8">
            <div className="space-y-1 text-center">
              <h4 className="text-[11px] font-black uppercase tracking-[0.4em] text-theme-muted">Estado de Energia</h4>
              <div className="h-1 w-10 bg-theme-accent/20 rounded-full mx-auto"></div>
            </div>
              ...
              {[1, 2, 3, 4, 5].map(v => (
                <button 
                  ...
                  className={`flex-1 aspect-square rounded-2xl flex items-center justify-center text-3xl transition-all active:scale-90 ${
                    ...
                    : 'bg-theme-bg border border-theme-border grayscale opacity-60 hover:opacity-100 hover:grayscale-0'
                  }`}
                >
                  {['😴', '🥱', '😐', '😊', '⚡'][v-1]}
                </button>
            ...
            <div className="space-y-4">
              <button 
                ...
                className={`w-full py-7 rounded-2xl font-black text-sm shadow-premium transition-all active:scale-[0.98] flex items-center justify-center gap-4 uppercase tracking-widest ${currentEnergy > 0 ? 'bg-theme-accent text-theme-card hover:opacity-95' : 'bg-theme-border text-theme-muted cursor-not-allowed opacity-50'}`}
              >
                <span className="material-symbols-outlined !text-2xl">timer</span>
                Foco Profundo
              </button>
              <p className="text-[9px] font-bold text-theme-muted text-center uppercase tracking-[0.2em] opacity-60">Sessão Pomodoro Recomendada</p>
            </div>
          </div>
        </div>
      </div>
